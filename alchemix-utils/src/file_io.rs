...
use crate::time::{self, system_time_to_iso};
...
pub fn get_created_date(path: &PathBuf) -> Option<String> {
    ...
    match time {
        Ok(time) => Some(system_time_to_iso(time)),
        Err(_) => None,
    }
}
...
pub fn get_modified_date(path: &PathBuf) -> String {
    ...
    match time {
        Ok(time) => system_time_to_iso(time),
...
pub fn get_metadata<P: AsRef<Path>>(path: P) -> HashMap<String, String> {
    ...
    if let Ok(value) = file_metadata.modified() {
        metadata.insert("modification_date".to_string(), system_time_to_iso(value));
    }
    if let Ok(value) = file_metadata.created() {
        metadata.insert("creation_date".to_string(), system_time_to_iso(value));
    }
    metadata
}
